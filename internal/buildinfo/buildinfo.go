// Package buildinfo internal/buildinfo/buildinfo.go
package buildinfo
...
// $ go build -mod=vendor -ldflags="-X 'github.com/0magnet/audioprism-go/internal/buildinfo.version=$(git describe)' -X 'github.com/0magnet/audioprism-go/internal/buildinfo.date=$(date -u "+%Y-%m-%dT%H:%M:%SZ")' -X 'github.com/0magnet/audioprism-go/internal/buildinfo.commit=$(git rev-list -1 HEAD)'" .
var (
	version   = unknown
	commit    = unknown
	date      = unknown
...
// $ go build -ldflags="-X 'github.com/0magnet/audioprism-go/internal/buildinfo.golist=$(go list -m -json -mod=mod github.com/0magnet/<repo>@<branch>)' -X 'github.com/0magnet/audioprism-go/internal/buildinfo.date=$(date -u "+%Y-%m-%dT%H:%M:%SZ")'" .
var golist string
...
func parseVersionInfo(ver string) {
	...
	if match := dateRegex.FindString(ver); match != "" {
		...
		// A pseudo-version built on a pre-release tag reads
		// v1.3.29-rc7.0.<timestamp>-<commit>: the .0 belongs to the scheme
		// rather than to the version, and leaving it prints v1.3.29-rc7.0.
		// Only reached when a timestamp was found, so a real version ending
		// in .0 is not touched.
...
func formatBuildDate(dateStr string) string {
	...
	// Fourteen characters is not fourteen digits. Slicing a non-numeric string
	// into date fields produces something that looks like a date and is not —
	// "not a date!!!!" becomes "not -a -daTte:!!:!!Z".
