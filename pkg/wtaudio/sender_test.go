...
func TestSenderChunksReassembleForTheBrowser(t *testing.T) {
	conn := &fakeConn{}
	s := NewSender(conn, 0)
	if s.MaxDatagramSize() != wsaudio.DefaultMaxDatagramSize {
		t.Fatalf("default datagram size is %d, want %d", s.MaxDatagramSize(), wsaudio.DefaultMaxDatagramSize)
	}
	...
	got := wsaudio.BytesToFloat32(out)
	if len(got) != len(in) {
...
func TestSenderShrinksToWhatTheStackAllows(t *testing.T) {
	...
	if got := wsaudio.BytesToFloat32(out); len(got) != len(in) {
...
func TestSenderRefusesAnUnusableLink(t *testing.T) {
	s := NewSender(&fakeConn{}, wsaudio.DatagramHeaderSize+3)
