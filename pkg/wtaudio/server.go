...
// as an OPTIONAL alternative to the WebSocket path in cmd/a/wasm and
// cmd/a/wasm. The WebSocket remains the default and is untouched by
// anything here; the native clients (pkg/ui/tcell, fyne, gomobile) stay on
// it too, because they are normally pointed at a server on the same
// machine and would pay for a QUIC stack in a cgo GUI build with nothing
// to show for it.
//
...
// spectrogram freezes and then fast-forwards. A spectrogram would rather
// lose 12 ms of audio than stutter, so it takes the transport that drops.
// The framing that makes that survivable is in pkg/wsaudio/datagram.go,
// shared with the browser.
//
// The payload bytes are identical to the WebSocket path's
// (wsaudio.Float32ToBytes), so the browser has exactly one decoder — and
// so a chaosrack client, which speaks the same framing over the same wire
// format, can read this server.
package wtaudio
...
// handlers do, so one tab going away cannot take the others down.
//
...
// WebSocket handlers follow, and the reason a capture is started per
