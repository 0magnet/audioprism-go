...
func TestServerStreamsToARealQUICClient(t *testing.T) {
	...
	for got == nil {
		...
		if len(dg) > wsaudio.DefaultMaxDatagramSize {
			t.Fatalf("a %d-byte datagram arrived, over the %d assumed", len(dg), wsaudio.DefaultMaxDatagramSize)
		}
		if payload := r.Push(dg); payload != nil {
			got = wsaudio.BytesToFloat32(payload)
		}
	}
