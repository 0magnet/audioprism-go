...
// stack says so. Pass 0 for wsaudio.DefaultMaxDatagramSize.
func NewSender(conn DatagramSender, maxSize int) *Sender {
	if maxSize <= 0 {
		maxSize = wsaudio.DefaultMaxDatagramSize
	}
...
//
// The samples are encoded by wsaudio.Float32ToBytes — the same function
// the WebSocket handlers call — so both transports put identical bytes on
...
func (s *Sender) Send(samples []float32) error {
	...
	payload := wsaudio.Float32ToBytes(samples)
	...
	for attempt := 0; attempt < 2; attempt++ {
		dgs := wsaudio.SplitDatagrams(s.id, payload, s.max)
		s.id++
		if dgs == nil {
