...
func Run(wid, hei, fpsRate, bSize int, fpsDisp bool, wsURL string) { //nolint:revive
	...
	// v3 has no PollEvent: the queue is an ordinary channel, and tcell closes
	// it when the screen is finalized, which ends this loop rather than leaving
	// it spinning on a nil event.
	for ev := range screen.EventQ() {
		switch ev := ev.(type) {
		case *tcell.EventKey:
			// Where the terminal reports key releases as well as presses, the
			// same keystroke arrives twice; quitting on the release would act
			// on the way out of a key meant for something else.
			if !ev.Pressed() {
				continue
			}
...
func renderLoop(fpsDisp bool) {
	...
	for range ticker.C {
		...
		if fpsDisp {
			...
			drawText(screen, termWidth-len(fpsStr)-2, 0, fpsStr, tcell.StyleDefault.Foreground(tcolor.Red))
		}
